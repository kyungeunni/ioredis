...
SentinelConnector.prototype.resolveMaster = function (client, callback) {
  ...
  client.sentinel('get-master-addr-by-name', this.options.name, function (err, result) {
    ...
    _this.updateSentinels(client, function (err) {
      ...
      var realHost = getAliasedHost(result[0]);
      callback(null, Array.isArray(result) ? { host: realHost, port: result[1] } : null);
    });
  });
};
...
function getAliasedHost(dockerIp){
  var hostMapper = {
    //master
    '172.17.0.7':'192.168.0.2',
    //slave
    '172.17.0.8':'192.168.0.2',
    //sentonel
    '172.17.0.9':'192.168.0.2'
  }

  return hostMapper[dockerIp] || null;
